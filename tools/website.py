...
class WebTools(Tool):
    ...
    def loadXML(self, filename, xmltree, sourcelist = None, **kwargs):
        for site_element in xmltree.iter(tag = "site"):
            ...
            if sourcelist.index("*") < 0 and sourcelist.index(site_name) < 0:
                continue
            if (site_element.get("disable")):
                logger.debug(f"Skipping {filename}:{site_name} disabled. {site_element.get("disable")}")
                continue
            logger.debug(f"Reading {filename}:{site_name}...")
            site = self._sites.get(site_name)
            site_args = Site.buildFromXML(site_element, **kwargs)
            if "delay" in site_element.attrib:
                site_args["delay"] = site_element.attrib["delay"]
            if site:
            ...
            site.Headers = { header_element.get("key"): header_element.text for header_element in site_element.findall("header") }
            site.Params = { param_element.get("key"): param_element.text for param_element in site_element.findall("param") }
    ...
    def getReport(self, site, target):
        ...
        if not respContent:
            logger.debug(f"No content returned by {site.DomainURL}")
            yield None
    ...
    def run(self, target, **kwargs):
        ...
        if sites_dict is None:
            yield ErrorReport(f"No sites to load from!")
        else:
            for site_name in sites_dict:
                site = sites_dict[site_name]
                #TODO proxy, useragent, botoutputrequested
...
class Site(AbstractContent):
    ...
    def loadArgs(self, **kwargs):
        ...
        Argument(s):
            ...
            delay -- the amount of seconds to wait between site retrieve calls.
    ...
    def getContent(self, target, source = None):
        ...
        if self._webretrievedelay:
            time.sleep(int(self._webretrievedelay))
